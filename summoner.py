## Summoner: id, name, league_id tier, division, date_scraped_peers, date_scraped_matches

from db_client import DbClient

class Summoner:
	def __init__(self, name, league_id, tier, division, date_scraped_peers = None, date_scraped_matches = None, id = None):
		self.id = id
		self.name = name
	...
	def from_object(cls, o):
		...
		return cls(name, league_id, tier, division, date_scraped_peers, date_scraped_matches, id)
	...
	def save(self):
		with DbClient() as db_client:
			##if already in db
			if self.id != None:
				db_client.update_summoner(self.id, self.name, self.tier, self.division, self.date_scraped_peers, self.date_scraped_matches)
			else:
				self.id = db_client.create_summoner(self.name, self.league_id, self.tier, self.division, self.date_scraped_peers, self.date_scraped_matches)
